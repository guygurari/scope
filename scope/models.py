...
def regression_fc_model(args, output_dim):
    ...
    if not args.fc_widths:
        model.add(_dense_layer(args, output_dim,
                               name='regression_map',
                               input_shape=(1,)))
    else:
...
def classification_convnet_model(args, input_shape, num_classes):
    ...
    model = keras.models.Sequential()
    model.add(_conv2d_layer(
        args, 32, (3, 3), padding='same', input_shape=input_shape))
    if args.batch_norm:
    ...
    if args.dense:
        ...
        if args.overparam > 0:
            for i in range(args.overparam):
                model.add(_dense_layer(args, args.cnn_last_layer,
    ...
    if args.overparam > 0:
        for i in range(args.overparam):
            model.add(_dense_layer(args, num_classes,
                                   name='output-overparam{}'.format(i + 1)))
