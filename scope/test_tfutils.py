...
class TestTensorFlowUtils(unittest.TestCase):
    ...
    def test_jacobian_dynamic_dim(self):
        ...
        with tf.Session() as sess:
            sess.run(tf.global_variables_initializer())
            jacobian = tfutils.jacobian(y, x)
            jacobian_actual = sess.run(
                jacobian, feed_dict={m: m_np, x: [4., -1.]})
            self.assertTrue(np.allclose(jacobian_actual, m_np))
    ...
    def test_gradients_and_hessians(self):
        ...
        with tf.Session() as sess:
            ...
            grad_norm_sqr_t = tf.reduce_sum(
                [tf.reduce_sum(g * g) for g in grad_t])
            ...
            results = sess.run(
                (loss_t, grad_t, grad_norm_sqr_t, term1_t, tr_hessian_t,
                 tr_hessian_2_t),
            ...
            self.assertAlmostEqual(
                grad_norm_sqr, grad_norm_sqr_result, precision)

            for term1_elem, term1_result_elem in zip(term1, term1_result[0]):
                self.assertAlmostEqual(
                    term1_elem, term1_result_elem, precision)
    ...
    def test_hessian_spectrum_lanczos(self):
        ...
        hess = tfutils.keras_compute_tensors(
            model, features, y, hess_t).reshape(p, p)
    ...
    def test_hessian_spectrum_with_matrix_vector_action(self):
        ...
        actual_eval, actual_evec = spec.compute_leading_ev(
            epsilon=1e-5,
            matrix_vector_action=lambda v, Hv: v - eta * Hv)
    ...
    def test_lanczos_eigsh(self):
        ...
        for i in range(k):
            ...
            act_v = actual_v[:, i]
            rtol = 1e-3
            self.assertTrue(np.allclose(exp_v, act_v, rtol=rtol) or
    ...
    def test_compute_sample_mean_tensor(self):
        ...
        actual_grad = tfutils.compute_sample_mean_tensor(
            model, batches, grad_t)
...
class TestMeasurements(unittest.TestCase):
    def test_gradient_measurement(self):
        ...
        y_test = np.sin(
            2 * np.pi * x_test[:, 0]).reshape((-1, 1)).astype(np.float32)
    ...
    def test_full_hessian_measurement(self):
        ...
        meas = measurements.FullHessianMeasurement(
            MockWriter(), model, 1, batches, None, 1)
